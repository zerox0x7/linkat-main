...
<head>
    ...
    <title>تخصيص الثيم - {{ \App\Models\Setting::get('store_name', config('app.name')) }}</title>
...
<body class="bg-[#0a0f1a] text-gray-100" style="overflow-y: scroll; -ms-overflow-style: none; scrollbar-width: none;">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-white flex items-center gap-3">
                            <div class="w-10 h-10 rounded-lg bg-gradient-to-r from-purple-500/20 to-pink-500/20 flex items-center justify-center">
                                <i class="ri-brush-3-line text-purple-400"></i>
                            </div>
                            تخصيص الثيم النشط
                        </h1>
                        <p class="text-gray-400 mt-1">قم بتخصيص صور وبيانات الثيم: <span class="text-primary font-semibold">{{ ucfirst($activeTheme) }}</span></p>
                    </div>
    ...
    <script>
        ...
        document.addEventListener('livewire:load', function () {
            Livewire.on('theme-saved', function () {
